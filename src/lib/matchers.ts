...
import { IndiciaDefaultError } from "../models/errors/indicia-default-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | IndiciaDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | IndiciaDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new IndiciaDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new IndiciaDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
